...
def main():
	...
	while (1):
		...
		if(Yolo!=c):
			...
			xx,yy,xxb,yyb = maptorealsense_andrecongnizesweetspot_andpickmango(yolo)
...
def maptorealsense_andrecongnizesweetspot_andpickmango(send1):
	...
	for jj in range (row1):
		...
		if (send2[jj][0] == "targetmango"):
				...
				if ( 390>X>105 and 268>Y>40):
					...
					yyy = ' '+str(Y)
 					break
		else if (send2[jj][0] == "badmango"):
				send22b = np.delete(send2, 0, 1)
