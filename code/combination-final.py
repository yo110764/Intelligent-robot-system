...
def main():
	...
	test_arm_2   =np.array([109.491,-488.961,80.569]) #mapping 
...
def maptorealsense(send1):

	send2 = np.reshape(send1, (-1, 4))
	xycoordinates = np.array([send2[0][2],send2[0][3]])
	X = float(xycoordinates[0])
	Y = float(xycoordinates[1])
	X = '%.2f'%X
	Y = '%.2f'%Y
	xxx = str(X)
	yyy = ' '+str(Y)
...
def maptoxyz(inputtt,xx,yy,corect_x1,corect_y1,corect_z1):
	...
	aftermap = np.fromstring(inputtt,dtype=float,sep=' ')
	worldz = aftermap
	worldz = float(worldz)
