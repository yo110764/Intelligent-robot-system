...
int main(int argc,char *argv[])
{
    ...
    check_error(e);
//1    printf("There are %d connected RealSense devices.\n", dev_count);
    if (0 == dev_count)
        return EXIT_FAILURE;
    ...
    while (1)
    {
        ...
        for (i = 0; i < num_of_frames; ++i)
        {
           ...
           //1 float dist_to_center = rs2_depth_frame_get_distance(frame, width / 2, height / 2, &e);
	    float dist_to_center = rs2_depth_frame_get_distance(frame, x, y, &e);
            check_error(e);

            // Print the distance
//1         printf("The camera is facing an object %.3f meters away.\n", dist_to_center);
	    printf("%.3f",dist_to_center);
	    break;
